function [Des,Des_Props_Hull,Des_Props_Wing,HullCoord] = FACaseI(Dt,Dh,Lh)
...
Volumes = [];
        ...
        [x,y,SectionalLengths] = HullForm(Lh,Dh,PercentArray,na,nf);
        HullCoord = [x,y];
        Lpmb = SectionalLengths(1);
        Lf = SectionalLengths(2);
        La = SectionalLengths(3);
        [WS_Hull,Vol_Hull,K1,K2] = HullAreaVolumeEstimator(nf,na,La,Lf,Lpmb,Lh,Dh);
        Cd_Hull = HullDragEstimator(v_AUV,WS_Hull,Vol_Hull,Lh,Dh,K2);
        frac = Vol_Hull./(0.25*pi*Dh^2*Lh);
        RatedPressure = rho_seawater*g*rateddepth; % Pa
        Thickness = RatedThickness(Dh,RatedPressure); % m 
        ID = Dh - 2*Thickness; % m, inner hull dia
        IL = Lh - 2*Thickness;  % m, inner hull length
        ...
        Wing(15) = Cd_wing;
        Wing(16) = Fd_wing;
        [Wt_Batteries_rm] = BallastProps([Dt,ri,re,theta_diff,Cd_Duct,Cd_Turbine,Cd_DTS_Moving,Cd_DTS_Opr,Cp_DTS],[Dh,Lh,frac,Cd_Hull,WS_Hull,Thickness,Vol_Hull,IVol_Hull],Wing,[v_rated,v_AUV,v_current]); % kg
        [EPropTot,time_tot_days,P_Turbine,P_AUV] = AUVPropulsion([Dt,ri,re,theta_diff,Cd_Duct,Cd_Turbine,Cd_DTS_Moving,Cd_DTS_Opr,Cp_DTS],[Dh,Lh,frac,Cd_Hull,WS_Hull,Thickness,Vol_Hull,IVol_Hull],Wing,[v_rated,v_AUV,v_current]); % MWh, days, kW, kW
        ...
        Des_Props_Hull = vertcat(Des_Props_Hull,[Dt,Dh,frac,Cd_Hull,WS_Hull,Vol_Hull,IVol_Hull,Thickness,Vol_Storage,Vol_Ballast]);
        Des_Props_Wing = vertcat(Des_Props_Wing,[Dt,Dh,Cd_wing,Fd_wing,bwing,bref,beff,ChordLength_Out,ChordThick_Out,ChordLength_In,ChordThick_In,Vol_Wing_Out,Wing(12),Wt_Wing,Aplanform_wing,Aref_wing,Aeff_wing]);
%         Des = vertcat(Des,[Dt,Dh,Lh,Wt_Batteries_rm,P_AUV,E_Cargo,EPropTot,E_Surplus,t_opr_days]);
        Des = vertcat(Des,[Dt,Dh,Lh,P_AUV,E_Cargo,EPropTot,E_Surplus,t_opr_days]);
end